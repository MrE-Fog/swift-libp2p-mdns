...
public class MulticastPeerDiscovery:Discovery, PeerDiscovery, LifecycleHandler {
    ...
    public var on: ((PeerDiscoveryEvent) -> ())?
    ...
    private typealias OutstandingQuery = (query:Question, callback:(DNS.Message) -> ())

    init(app:Application, interfaceAddress:SocketAddress? = nil, multicastGroup:SocketAddress = try! SocketAddress(ipAddress: "224.0.0.251", port: 5353)) {
    //init(on loop:EventLoop, peerID:PeerID, interfaceAddress:String, multicastGroup:String = "224.0.0.251:5353") {
        ...
        if let ia = interfaceAddress {
            if ia.protocol == .inet {
                ...
                self.interfaceAddressV6 = try! MulticastPeerDiscovery.interfaceAddress(forCodec: .ip6, onDevice: MulticastPeerDiscovery.interfaceAddressDevice(ia)!)!
            } else if ia.protocol == .inet6 {
                self.interfaceAddressV6 = ia
                self.interfaceAddressV4 = try! MulticastPeerDiscovery.interfaceAddress(forCodec: .ip4, onDevice: MulticastPeerDiscovery.interfaceAddressDevice(ia)!)!
            } else {
    ...
    private static func defaultInterfaceAddress(codec:MultiaddrProtocol, deviceName:String = "en0") throws -> SocketAddress? {
    ...
    private static func interfaceAddress(forCodec codec:MultiaddrProtocol, onDevice targetDevice:NIONetworkDevice) throws -> SocketAddress? {
        guard codec == .ip4 || codec == .ip6 else { return nil }
        return try System.enumerateDevices().compactMap({ device in
    ...
    public func start() throws {
        ...
        case .stopped:
            ...
            self.mdnsHost = try self.makeUDPMulticastHost(self.group!, interfaceAddress: self.interfaceAddressV4, onMulticastGroup: self.multicastGroup)
            ...
            application.events.on(self, event: .listen( { [unowned self] pid, newAddress in
            ...
            application.events.on(self, event: .listenClosed( { [unowned self] pid, oldAddress in
                guard pid == self.peerID.b58String else { return }
                let _ = self.eventLoop.submit {
            ...
            self.serviceDiscoveryTask = self.eventLoop.scheduleRepeatedAsyncTask(initialDelay: .milliseconds(500), delay: .seconds(5), notifying: nil) { task in
                self.logger.trace("Querying for peers")
                return self.queryForPeers().map { multiaddresses in
                    guard multiaddresses.count > 0 else { self.logger.trace("No Local Peers Found"); return }
...
//                    }
        ...
        case .stopping:
            self.logger.warning("The mDNS service is attempting to stop, please wait until the service has fully stopped before attempting to start")
    ...
    private func multiaddressesToPeerInfos(_ mas:[Multiaddr]) -> [PeerInfo] {
        ...
        for peer in uniquePeers {
            ...
            pInfos.append(
                PeerInfo(
                    peer: pid,
                    addresses: mas.filter({ $0.getPeerID() == peer }).fixSameHostAddresses(ip4: self.interfaceAddressV4.ipAddress!)
                )
            )
        }
        return pInfos
    ...
    private func makeUDPMulticastHost(_ group:EventLoopGroup, interfaceAddress:SocketAddress, onMulticastGroup:SocketAddress) throws -> Channel {
    ...
    private func extractMultiaddressFromAdditionalRecords(_ records:[ResourceRecord]) -> [Multiaddr] {
        ...
        guard let pid = peerID else { self.logger.trace("Dropping mDNS message due to no PeerID"); return [] }
        guard pid != self.peerID.b58String else { self.logger.trace("Dropping self issued mDNS message"); return [] }
        guard !protos.isEmpty else { self.logger.trace("Dropping mDNS message due to no protocols in text record"); return [] }
        ...
        if let ip4 = ip4Address {
            ...
            multiaddresses.append(contentsOf: protos.compactMap { proto, port in
                if proto == "ws" {
    ...
    public func registerService(name:String, protocol proto:String = "udp", serviceName:String = "p2p") -> EventLoopFuture<Bool> { //"instance-name"._name._protocol -> chat-room-name._p2p._udp
        guard self._state == .started else { return self.eventLoop.makeFailedFuture(Errors.mdnsServiceNotStarted) }
        
        return self.eventLoop.submit {
            guard proto == "udp" || proto == "tcp" else { self.logger.warning("Invalid protocol"); return false }
            guard self.registeredServices[name] == nil else { self.logger.warning("Service `\(name)` already registered"); return false }

            /// Attempt to register the custom service
            var dnsService:DNSServiceRef? = nil
            let flags:DNSServiceFlags = DNSServiceFlags()
            DNSServiceRegister(&dnsService, flags, 0, name, "_\(serviceName)._\(proto)", "", "", UInt16(3338), 0, "", nil, nil)
...
//                }
            ...
            DNSServiceRegister(&dnsService, flags, 0, self.peerID.b58String, "_p2p._udp", "", "", 3338, TXTRecordGetLength(&txtRecord), TXTRecordGetBytesPtr(&txtRecord), nil, nil)
    ...
    private func newQueryResponse(_ query:Question) -> EventLoopFuture<[DNS.Message]> {
        ...
        let handler = OutstandingQuery(query: query, callback: { msg in
        ...
        self.mdnsHost.writeAndFlush(AddressedEnvelope(remoteAddress: multicastGroup, data: try! msg.serialize()), promise: nil)
...
private extension Array where Element == Multiaddr {
    func fixSameHostAddresses(ip4:String) -> [Multiaddr] {
...
internal extension Multiaddr {
    func replace(address:String, with replacement:String, forCodec codec:MultiaddrProtocol) -> Multiaddr? {
        if self.description.contains("\(codec.name)/\(address)") {
            if let new = try? Multiaddr(self.description.replacingOccurrences(of: "\(codec.name)/\(address)", with: "\(codec.name)/\(replacement)")) {
                return new
            } else {
                return nil
            }
